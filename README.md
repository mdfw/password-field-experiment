...
* The password fields should be actual password fields at load. 'show password' will flip that to a regular text field if necessary.
* Should have 2 password fields to verify that the user correctly entered their password, *unless* a user chooses the 'show password' option. Then we don't need to verify and can take the second one out.
...
I'll admit that I didn't fully understand it until I tried this interactive demo: [https://www.jasondavies.com/bloomfilter/](https://www.jasondavies.com/bloomfilter/) which is based on his own bloom filter written in JS: [https://github.com/jasondavies/bloomfilter.js](https://github.com/jasondavies/bloomfilter.js).
...
We need a list of common passwords. Luckily, [Daniel Miessler](https://github.com/danielmiessler) put together a number of lists of passwords in his [SecLists Github repo](https://github.com/danielmiessler/SecLists/tree/master/Passwords). I pulled down the 100, 500, 1,000, 10,000 and 100,000 top lists to run tests against. 

_Note: You can run your own tests with the included `createbloom.js` and `validatebloom.js` files. These aren't sophisticated scripts so read the source if you want to know how to use them._
...
1. Two password fields are initially displayed. 
2. A Show password button/switch is available. 
    * When switched
        1. The 'verify' password field fades out
        2. The 'main' password field switches to a text box
        3. The 'show password' button switches to 'Mask password', which reverses these changes.
...
4. As user types:
    ...
    - If passwords above 8 but less than 20, show 20 greyed and others actively as 
    - If over 20, show yes for 20, grey out others
    - If over 256, show overage
    - If the company name is a substring in the password, show an error.
...
Also: You could use this method with the larger files on a node server backend - bloom filters are supposed to be more efficient than an array lookup. I did not test that but might be a good followup.

## Implementation Notes

One issue I ran into was making the password typing visibility accessible. I initially used a toggle password field code from here: http://www.mburnette.com/blog/toggle-password-field-text-visibility but the problem with that is it keeps the text field (the visible password) and the password field (the 'hidden' text) in sync. This, however, causes the browser not to call out the typed text when it's visible because it's being kept in sync between the two areas. I had to rewrite by swapping out the hidden field and syncing the content at that point.
...
## Notes
<a name="note1">**Aside:**</a> While this is presented nice and step by step, the actual process included significant trial and error, walk-aways, _WHAT?!_'s and a few _aha!_ moments. I mention this to allow anyone new to this that are feeling imposter syndrome to not worry, it wasn't this clean. It just makes for a cleaner demo this way.
